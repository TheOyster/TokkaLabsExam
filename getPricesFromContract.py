...
fee_tier = 500
starting_block = 17618642

# Iteratively find the required ETH amount
eth_amount = 25.618313400  # Starting point
increment = 0.000000001  # Small increment to adjust ETH amount
desired_usdc = 50000  # Target USDC amount

while True:
    usdc_output = get_usdc_from_eth(quoter_contract, eth_amount, eth_address, usdc_address, fee_tier, starting_block)
    print(f"Testing {eth_amount:.9f} ETH -> USDC: {usdc_output:.4f}")
    if usdc_output >= desired_usdc:
        print(f"Required ETH to obtain at least {desired_usdc} USDC: {eth_amount:.3f}")
        break
    eth_amount += increment  # Incrementally adjust the ETH amount
